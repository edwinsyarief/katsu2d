...
type Line struct {
	...
	points []Vector // Array of points defining the line path
	...
	debugPoints    []Vector        // The points used for building the mesh after processing
...
func (self *Line) AddPoint(pos Vector) {
...
func (self *Line) SetPoint(index int, position Vector) {
...
func (self *Line) GetPoints() []Vector {
	return self.points
}
...
func (self *Line) generateSplinePoints(points []Vector, precision int) []Vector {
	if len(points) < 2 {
		return points
	}
	newPoints := make([]Vector, 0)
...
func (self *Line) resamplePoints(points []Vector, step float64) []Vector {
	if len(points) < 2 || step <= 0 {
		return points
	}
	newPoints := make([]Vector, 0, len(points))
	newPoints = append(newPoints, points[0])
	distanceNeeded := step
	for i := 0; i < len(points)-1; i++ {
		p1 := points[i]
