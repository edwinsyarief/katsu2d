...
func (self *noise) extrapolate2(xsb, ysb int32, dx, dy float64) float64 {
	index := self.perm[(int32(self.perm[xsb&0xFF])+ysb)&0xFF] & 0x0E
	return float64(gradients2D[index])*dx + float64(gradients2D[index+1])*dy
}

func (self *noise) extrapolate3(xsb, ysb, zsb int32, dx, dy, dz float64) float64 {
	index := self.permGradIndex3D[(int32(self.perm[(int32(self.perm[xsb&0xFF])+ysb)&0xFF])+zsb)&0xFF]
	return float64(gradients3D[index])*dx + float64(gradients3D[index+1])*dy + float64(gradients3D[index+2])*dz
}

func (self *noise) extrapolate4(xsb, ysb, zsb, wsb int32, dx, dy, dz, dw float64) float64 {
	index := self.perm[(int32(self.perm[(int32(self.perm[(int32(self.perm[xsb&0xFF])+ysb)&0xFF])+zsb)&0xFF])+wsb)&0xFF] & 0xFC
