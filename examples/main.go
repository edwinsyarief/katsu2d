...
func NewGame() *Game {
	...
	tm.Add(tree) // ID 3: "tree_trunk"

	leaves := ebiten.NewImage(16, 16)
	leaves.Fill(color.RGBA{R: 34, G: 139, B: 34, A: 255})
	tm.Add(leaves) // ID 4: "tree_leaves"

	path := ebiten.NewImage(16, 16)
	path.Fill(color.RGBA{R: 188, G: 143, B: 143, A: 255})
	tm.Add(path) // ID 5: "path"

	playerImg := ebiten.NewImage(16, 16)
	playerImg.Fill(color.White)
	playerTexID := tm.Add(playerImg) // ID 6: Player
	...
	// 3. TopGridRenderSystem draws the top-most layer, which appears over everything else.
	g.engine.AddOverlayDrawSystem(katsu2d.NewTopGridRenderSystem(tm))
