...
// It holds three grids: a lower grid for the ground, an upper grid for objects
// that can be sorted with sprites, and a top grid for overlays.
...
func LoadFromJSON(data []byte) (*DualGridTileMap, error) {
	...
	// Populate top grid
	if len(md.TopGrid) != md.Width*md.Height {
		// Allow top_grid to be optional for backward compatibility
		if len(md.TopGrid) > 0 {
			return nil, fmt.Errorf("top_grid size (%d) does not match map dimensions (%d x %d = %d)", len(md.TopGrid), md.Width, md.Height, md.Width*md.Height)
		}
	} else {
		for i, tileID := range md.TopGrid {
			if tileID == 0 {
				continue // 0 is an empty tile
			}
			tile, ok := tilemap.tileset[tileID]
